...
python /cephyr/users/mircog/Vera/DPDM/dataset_tool.py --source=$TMPDIR/ILSVRC/Data/CLS-LOC/train --dest="/mimer/NOBACKUP/groups/naiss2024-22-432/processed/imagenet.zip" --resolution=32x32 --transform=center-crop
if [[ $? -ne 0 ]]; then
    echo "ERROR: Dataset processing failed!"
    exit 1
fi
echo "Dataset processing complete."
echo "Computing FID statistics..."
python /cephyr/users/mircog/Vera/DPDM/compute_fid_statistics.py --path "/mimer/NOBACKUP/groups/naiss2024-22-432/processed/imagenet.zip" --file imagenet.npz --fid_dir "/cephyr/users/mircog/Vera/DPDM/assets/stats"
if [[ $? -ne 0 ]]; then
    echo "ERROR: FID computation failed!"
    exit 1
fi
echo "FID computation complete."
